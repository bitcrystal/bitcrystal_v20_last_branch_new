	...
	unsigned char set=0;
       	if(fps!=0)
        {
        	while(fi<free_pointers_size)
                {
                    if(fps[fi].is_free==1&&fps[fi].is_readed==0&&fps[fi].address>0&&fps[fi].address>=zz->start_address&&fps[fi].address<=current_address&&fps[fi].size>=zz->size)
                    {
